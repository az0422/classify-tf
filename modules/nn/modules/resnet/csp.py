...
from .residual import ResNet, SEResNet, ResNet2L
...
class CSPResNet2L3C(Layer):
    def __init__(self, in_channels, out_channels, n=1, expand=0.5):
        ...
        self.m = Sequential([
            ResNet2L(channels_h, channels_h, expand) for _ in range(n)
        ])
...
class CSPResNet2L2C(Layer):
    def __init__(self, in_channels, out_channels, n=1, expand=0.5, kernel=3):
        ...
        self.conv2 = Conv(out_channels, out_channels, 1, 1)
        self.m = Sequential([
            ResNet2L(out_channels // 2, out_channels // 2, expand, kernel) for _ in range(n)
        ])
